...
def calculate_indicators(df):
    ...
    df['is_volume_trend_up'] = df['volume'] > df['volume'].shift(1) * (1 + 25.0 / 100)

    df['max_volume'] = df['volume'].cummax()
    df['vol_strength'] = (df['volume'] / df['max_volume']) * 100
    df['is_high_vol_strength'] = df['vol_strength'] >= 150.0
...
def check_signals(df, buy_prices, buy_amounts, in_position):
    ...
    required_drop = config['DCA_PRICE_DROP_START'] + (dca_step * config['DCA_PRICE_DROP_INCREMENT'])
    ...
    if just_sold:
        ...
        if avg_buy_price is not None and avg_buy_price > 0:
            ...
            long_signal = (
                is_bearish 
                and latest['close'] < avg_buy_price
                and price_drop_percent >= required_drop
            )
        else:
            long_signal = False
    ...
    short_signal = False
    if in_position:
        if (has_min_profit and 
            ((is_bearish and is_weakening) or latest['is_price_drop_signal'] or stopLossTriggered or 
             (latest['macd_line'] < latest['signal_line']))
        ):
            short_signal = True
...
class TradingBot:
    def __init__(self, gui_queue):
        ...
        # Información adicional para la GUI
        self.next_dca_drop = 0.0
        self.coin_balance = 0.0
        self.approx_sell_price = 0.0
        self.approx_sell_profit = 0.0
        ...
        while self.running:
            try:
                ...
                (
                    ...
                    required_drop,
                    avg_buy_price
                ) = check_signals(df, buy_prices, buy_amounts, in_position)

                self.avg_buy_price = avg_buy_price

                # Guardamos la info para mandar a la GUI
                self.next_dca_drop = required_drop
                self.coin_balance = position_amt

                # (FIX) Verificamos que self.avg_buy_price no sea None antes de compararla
                if self.avg_buy_price is not None and self.avg_buy_price > 0:
                    self.approx_sell_price = self.avg_buy_price * (1 + config['MIN_PROFIT_PERCENT'] / 100.0)
                else:
                    self.approx_sell_price = 0.0

                # (FIX) Aquí también
                if position_amt > 0 and self.avg_buy_price is not None and self.avg_buy_price > 0:
                    diff_price = (self.approx_sell_price - self.avg_buy_price)
                    self.approx_sell_profit = diff_price * position_amt
                else:
                    self.approx_sell_profit = 0.0
                ...
                self.send_gui_update(
                    ...
                    next_dca_drop=self.next_dca_drop,
                    coin_balance=self.coin_balance,
                    approx_sell_price=self.approx_sell_price,
                    approx_sell_profit=self.approx_sell_profit,
                )
                time.sleep(1)
            except Exception as e:
...
class BotGUI:
    def __init__(self, master, bot):
        ...
        self.log_messages = []  # Para mostrar máximo 40 líneas
    ...
    def create_status_tab(self):
        ...
        # Campos nuevos
        ttk.Label(left_frame, text="Próx. Bajada DCA (%):").grid(row=13, column=0, sticky=tk.W, padx=2, pady=2)
        self.next_dca_drop_var = tk.StringVar(value="---")
        self.next_dca_drop_label = ttk.Label(left_frame, textvariable=self.next_dca_drop_var)
        self.next_dca_drop_label.grid(row=13, column=1, sticky=tk.W, padx=2, pady=2)

        ttk.Label(left_frame, text="Balance Cripto:").grid(row=14, column=0, sticky=tk.W, padx=2, pady=2)
        self.coin_balance_var = tk.StringVar(value="---")
        self.coin_balance_label = ttk.Label(left_frame, textvariable=self.coin_balance_var)
        self.coin_balance_label.grid(row=14, column=1, sticky=tk.W, padx=2, pady=2)

        ttk.Label(left_frame, text="Precio Venta Aproximado:").grid(row=15, column=0, sticky=tk.W, padx=2, pady=2)
        self.approx_sell_price_var = tk.StringVar(value="---")
        self.approx_sell_price_label = ttk.Label(left_frame, textvariable=self.approx_sell_price_var)
        self.approx_sell_price_label.grid(row=15, column=1, sticky=tk.W, padx=2, pady=2)

        ttk.Label(left_frame, text="Ganancia Venta Aprox.:").grid(row=16, column=0, sticky=tk.W, padx=2, pady=2)
        self.approx_sell_profit_var = tk.StringVar(value="---")
        self.approx_sell_profit_label = ttk.Label(left_frame, textvariable=self.approx_sell_profit_var)
        self.approx_sell_profit_label.grid(row=16, column=1, sticky=tk.W, padx=2, pady=2)
    ...
    def update_trades_plot(self):
        ...
        if buys:
            ...
            self.ax.scatter(buy_times, buy_prices, marker='^', color='green', label='Compra', s=80, zorder=5)

        if sells:
            sell_times, sell_prices = zip(*sells)
            self.ax.scatter(sell_times, sell_prices, marker='v', color='red', label='Venta', s=80, zorder=5)

        if self.bot.avg_buy_price is not None:
            self.ax.axhline(y=self.bot.avg_buy_price, color='orange', linestyle='--', label='Precio Promedio Compra', zorder=4)
    ...
    def update_gui(self):
        while not self.queue.empty():
            ...
            if isinstance(msg, dict):
                ...
                # Campos nuevos
                if 'next_dca_drop' in msg:
                    self.next_dca_drop_var.set(f"{msg['next_dca_drop']:.2f}%")
                if 'coin_balance' in msg:
                    self.coin_balance_var.set(f"{msg['coin_balance']:.6f}")
                if 'approx_sell_price' in msg:
                    self.approx_sell_price_var.set(f"{msg['approx_sell_price']:.5f}")
                if 'approx_sell_profit' in msg:
                    self.approx_sell_profit_var.set(f"{msg['approx_sell_profit']:.5f} USDT")
